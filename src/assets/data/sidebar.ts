import { ItemSidebar } from '../../app/interfaces/item-sidebar';

export const secretaria: ItemSidebar[] = [
  { name: 'Estadísticas', route: 'estadisticas', icon: 'estadisticas.svg' },
  { name: 'Quejas', route: 'quejas', icon: 'quejas.svg' },
  { name: 'Buscar conductores', route: 'buscar-conductores', icon: 'buscarconductores.svg' },
  { name: 'Usuarios', route: 'usuarios', icon: 'usuarios.svg' },
  { name: 'Perfil de usuario', route: 'perfil-de-usuario', icon: 'perfilusuarios.svg' },
  { name: 'Parque automotor', route: 'parque-automotor', icon: 'parqueautomotor.svg' },
  { name: 'Parámetros', route: 'parametros', icon: 'parametros.svg' },
]

export const empresa: ItemSidebar[] = [
  { name: 'Estadísticas', route: 'estadisticas', icon: 'estadisticas.svg' },
  { name: 'Buscar conductores', route: 'buscar-conductores', icon: 'buscarconductores.svg' },
  { name: 'Agregar conductor', route: 'agregar-conductor', icon: 'agregarconductor.svg' },
  { name: 'Desvinculaciones', route: 'desvinculaciones', icon: 'history.svg' },
  { name: 'Documentos vencidos', route: 'documentos-vencidos', icon: 'fechasvencidas.svg' },
  { name: 'Perfil de empresa', route: 'perfil-de-empresa', icon: 'perfilempresas.svg' },
  { name: 'Perfil de usuario', route: 'perfil-de-usuario', icon: 'perfilusuarios.svg' },
  { name: 'Alertas', route: 'alertas', icon: 'incidencias.svg' },
  { name: 'Parque automotor', route: 'parque-automotor', icon: 'parqueautomotor.svg' },
  { name: 'Usuarios', route: 'usuarios', icon: 'user.svg' },
]

export const auxiliar: ItemSidebar[] = [
  { name: 'Estadísticas', route: 'estadisticas', icon: 'estadisticas.svg' },
  { name: 'Buscar conductores', route: 'buscar-conductores', icon: 'buscarconductores.svg' },
  { name: 'Agregar conductor', route: 'agregar-conductor', icon: 'agregarconductor.svg' },
  { name: 'Desvinculaciones', route: 'desvinculaciones', icon: 'history.svg' },
  { name: 'Documentos vencidos', route: 'documentos-vencidos', icon: 'fechasvencidas.svg' },
  { name: 'Perfil de empresa', route: 'perfil-de-empresa', icon: 'perfilempresas.svg' },
  { name: 'Perfil de usuario', route: 'perfil-de-usuario', icon: 'perfilusuarios.svg' },
  { name: 'Alertas', route: 'alertas', icon: 'incidencias.svg' },
]
